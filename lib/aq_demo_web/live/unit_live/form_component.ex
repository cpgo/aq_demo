defmodule AqDemoWeb.UnitLive.FormComponent do
  use AqDemoWeb, :live_component

  alias AqDemo.Market

  @impl true
  def update(%{unit: unit} = assigns, socket) do
    changeset = Market.change_unit(unit)
  ...
  def handle_event("validate", %{"unit" => unit_params}, socket) do
    changeset =
      socket.assigns.unit
      |> Market.change_unit(unit_params)
  ...
  defp save_unit(socket, :edit, unit_params) do
    case Market.update_unit(socket.assigns.unit, unit_params) do
      {:ok, _unit} ->
        {:noreply,
         socket
  ...
  defp save_unit(socket, :new, unit_params) do
    case Market.create_unit(unit_params) do
      {:ok, _unit} ->
        {:noreply,
         socket
